...
# ── Public subnets (1 per AZ) — ALB, NLB ────────────────────────────────────
...
# ── Private subnets (1 per AZ) — ECS tasks, Aurora ──────────────────────────
...
# ── Elastic IPs + NAT Gateways (1 per AZ for HA) ────────────────────────────
...
# ECS tasks: accept from ALB (8080) and NLB (8583), plus all egress
resource "aws_security_group" "ecs" {
  ...
  # NLB uses VPC CIDR (NLB does not have security groups)
  ingress {
...
# Aurora: accept from ECS tasks only
